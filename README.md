...
- **ОС:** macOS 15+
- **Компилятор:** clang
- **Тесты:** IO Test Framework; санитайзеры включены
...
make TASK=ТТ-NN run     # Запустить решение задачи
make TASK=ТТ-NN test    # Запустить тесты задачи
make TASK=ТТ-NN demo    # Запустить демо программу
...
1. Откройте `ТТ-NN-taskname.readme.md` и `ТТ-NN-taskname.c` в редакторе
2. Прочитайте задание и теорию из `ТТ-NN-taskname.readme.md`
...
make TASK=01-01 test    # Тесты должны пройти
make TASK=01-01 run     # Запустить готовую программу
make TASK=01-01 demo    # Запустить демо (если есть)

# Или из папки задачи
make test        # Тесты должны пройти
make build       # Создаёт обычный исполняемый файл
make debug       # Создаёт файл для отладки
./build/ТТ-NN-taskname  # Запуск программы вручную
```

### Шаг 5: Отладка в VS Code (опционально)

1. Откройте `ТТ-NN-taskname.c` в VS Code
